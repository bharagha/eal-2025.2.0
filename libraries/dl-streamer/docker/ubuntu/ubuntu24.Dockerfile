...
RUN \
    apt-get update && \
    apt-get install -y -q --no-install-recommends build-essential=\* wget=\* git=\* cmake=\* pkg-config=\* meson=\* libdrm-dev=\* automake=\* libtool=\* curl=\* gpg=\* ca-certificates=\* libtbb12=\* git=\* python3-venv=\* && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*
