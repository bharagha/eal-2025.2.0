        ...
        if (device.find("GPU") != std::string::npos && context) {
            ...
            if (va_display) {
                ...
                if (approach == 1) {
                    ...
                    HRESULT hr = S_OK;
                    hr = D3D11CreateDevice( nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, D3D11_CREATE_DEVICE_DEBUG, &featureLevels[0], numFeatureLevels - 1,
                                    D3D11_SDK_VERSION, &pd3dDevice, &featureLevel, &pImmediateContext );
                    assert(hr == S_OK);
                } else if (approach == 2) {
                    gint64 adapter_luid = 52444;
                ...
                _remote_context = ov::intel_gpu::ocl::D3DContext(core, pd3dDevice, tile_id);
                return;
